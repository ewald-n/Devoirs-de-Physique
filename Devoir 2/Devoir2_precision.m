...
for opt = options
    ...
    for essai = 1:4 
        ...
        [m_tests, exec_times, precision] = m_opt(opt, rbi, vbi, wbi, epsilon, m_max);

        nb_intervalles = 2.^(m_tests-1);
        
        nexttile
        plot(nb_intervalles,precision, nb_intervalles, epsilon*ones(length(m_tests)), '--' )
        yscale log
        title(sprintf("Précisions spatiales en fonction du nombre d'intervalles, essai %d, option %d", essai, opt))
        ylabel("Précision en m")
        xlabel("Nombre d'intervalles")
        legend('Erreur en norme', "Spécification")
        
        nexttile
        plot(nb_intervalles,exec_times)
        yscale log
        ylabel("Temps d'execution en s")
        xlabel("Nombre d'intervalles")
        title(sprintf("Temps d'éxecution en fonction du nombre d'intervalles, essai %d, option %d",essai, opt))
    end
end

function [ vbf, ti, x, y, z] = Devoir2(option, r_init, v_init, w_init)
...
%Version allégée de la fonction précédente qui ne prend pas en compte les
%critères d'arrêt dûs à l'environnement et qui n'enregistre par les
%positions successives, afin d'étudier la précision nécessaire.
%On ne prend en compte que la précision en position
function rbf = Devoir2_controleerreur(option, r_init, v_init, w_init,m)
    % Définition de l'énumération d'index XYZ pour vec 3D
    Index3D = struct('X', 1, 'Y', 2, 'Z', 3);
    ...
    % Résultats finaux
    vbf = qsol_i(IndexQ.v,:);                  % Vitesse finale
    rbf = qsol_i(IndexQ.r,:);                  % Vitesse finale
...
%Fonction qui recherche l'indice m optimal pour atteindre la précision
%epsilon demandée
function [m_tests, exec_times, precision] = m_opt(option, r_init, v_init, w_init, epsilon, m_max)
    indice = 1;
    m = 7; %valeur minimale calculée à partir du nombre minimal d'intervalles demandé
    rbi = 0;

    tic;
    rbf = Devoir2_controleerreur(option, r_init, v_init, w_init,m);
    time = toc;
    E_r = rbf - rbi;

    m_tests(indice) = m;
    exec_times(indice) = time;
    
    precision(indice) = norm(E_r);

    % precisions_x(indice) = abs(E_r(1));
    % precisions_y(indice) = abs(E_r(2));
    % precisions_z(indice) = abs(E_r(3));

    condition = critere(E_r,epsilon);

    while condition == false && m<=m_max
        indice = indice+1;
        m = m+1 ;
        rbi = rbf ;
...
function condition = critere(E_r, epsilon)
    condition = norm(E_r)<epsilon;
end
